...
import { Ticket, TicketStatus, User, UserRole, Project } from '../types';
import { formatDate } from '../utils';
import { Clock, MessageSquare, Paperclip, ChevronRight, User as UserIcon, Calendar, Pencil, Trash2, Briefcase } from 'lucide-react';
...
const TicketList: React.FC<Props> = ({ tickets, projects, currentUser, users, onSelect, onEdit, onDelete }) => {
  if (tickets.length === 0) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="hidden lg:grid grid-cols-[3rem_minmax(0,1.5fr)_minmax(0,0.8fr)_6rem_6rem_7rem_7rem_3rem] gap-4 px-5 py-3 bg-white border border-slate-200 rounded-2xl text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3">
      ...
      <div className="grid grid-cols-1 gap-3">
        {tickets.map(ticket => {
          const projectName = projects.find(p => p.id === ticket.projectId)?.name || '-';
          return (
            <div
              key={ticket.id}
              className="group bg-white border border-slate-200 lg:border-slate-100 rounded-2xl lg:rounded-2xl p-4 transition-all hover:border-blue-300 hover:shadow-xl hover:shadow-blue-500/5 cursor-pointer"
            >
              {/* Mobile/Tablet Card Layout */}
              <div className="lg:hidden" onClick={() => onSelect(ticket.id)}>
                <div className="flex justify-between items-start mb-4">
                  <div className="flex flex-col gap-1">
                    <span className="text-xs font-mono font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded w-fit">{ticket.id}</span>
              ...
              {/* Desktop Row Layout */}
              <div className="hidden lg:grid grid-cols-[3rem_minmax(0,1.5fr)_minmax(0,0.8fr)_6rem_6rem_7rem_7rem_3rem] gap-4 items-center px-5 py-3" onClick={() => onSelect(ticket.id)}>
                ...
                <div className="col-span-1">
                  <span className="text-xs font-bold text-slate-500 truncate block" title={projectName}>
                    {projectName}
                  </span>
                </div>
                <div className="col-span-1">
