...
import {
    ...
    Settings, Database, LogOut, FileText
} from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({
    ...
    const NavItem = ({ icon: Icon, label, targetView, adminOnly = false, supportOrAdmin = false }: any) => {
        ...
        return (
            <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all font-medium ${view === targetView ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/40' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                    }`}
            >
                <Icon size={18} />
    ...
    return (
        <div className="flex min-h-screen bg-slate-50">
            ...
            <aside className={`fixed inset-y-0 left-0 w-64 bg-slate-900 text-white flex flex-col z-50 transition-transform duration-300 transform lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="p-6 border-b border-slate-800 flex justify-between items-center">
                    <h1 className="text-xl font-bold flex items-center gap-2 cursor-pointer" onClick={() => changeView('list')}>
                        <span className="bg-blue-600 p-1.5 rounded-lg text-white shadow-lg shadow-blue-500/20">nu</span>
                        ServiceDesk
                    </h1>
                    <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-2 hover:bg-slate-800 rounded-lg"><X size={20} /></button>
                </div>

                <nav className="flex-1 min-h-0 p-4 space-y-1 overflow-y-auto custom-scrollbar pb-24">
                    ...
                    <div className="pt-6 pb-2 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Report</div>
                    ...
                    <div className="pt-6 pb-2 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">Management</div>
                    <NavItem icon={Briefcase} label="프로젝트 관리" targetView="projects" supportOrAdmin />
                    <NavItem icon={FileText} label="운영정보 관리" targetView="operationManagement" supportOrAdmin />
                    ...
                    <div className="pt-6 pb-2 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em]">System</div>
                    ...
                    <div className="px-3 pt-4 border-t border-slate-800 mt-4">
                        <button
                            ...
                            className="w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all font-medium text-red-400 hover:bg-red-500/10 hover:text-red-300"
                        >
                            <LogOut size={18} />
                ...
                <div onClick={() => changeView('profile')} className={`p-3 bg-slate-800/50 m-4 rounded-xl cursor-pointer hover:bg-slate-800 transition-all border border-slate-700/30 group ${view === 'profile' ? 'ring-2 ring-blue-500 border-blue-500' : ''}`}>
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-lg bg-blue-600 flex items-center justify-center font-bold shrink-0 shadow-lg group-hover:scale-105 transition-transform text-base">
                            {currentUser.name[0]}
                        </div>
